...
public:
    VertexBufferTpl() : bufferObject_(0), dataType_(0), vertices_(0) {}
    virtual ~VertexBufferTpl() { destroy(); }
    ...
    bool create(u64 vertexNum, T1** pVertexArray)
    {
        ...
        // GPU側の頂点領域を確保
        glBufferData(GL_ARRAY_BUFFER, vertexNum * sizeof(T1),
                     (void**)pVertexArray, GL_STATIC_DRAW);
     ...
     *
     */
    void setAttribute(s32 locationIndex, u32 elemNum, const Type& type,
                      boolean normalize, u32 elemOffset)
    {
        // 頂点要素を attribute 変数から参照できるようにする
        glEnableVertexAttribArray(locationIndex);
        glVertexAttribPointer(
            locationIndex,               // シェーダ変数の番号
            elemNum,                     // 頂点要素数
            type,                        // 型
            normalize,                   // 正規化するかどうか
            sizeof(T1),                  // 頂点型のサイズ(stride)
            reinterpret_cast<GLvoid*>(elemOffset));        // 要素の頂点型内でのオフセット
    }

    void bind() { glBindBuffer(GL_ARRAY_BUFFER, bufferObject_); }
...
public:
     ...
     */
    bool create(u64 vertexNum, s32 attribNum, s32 atrribSize,
                void** pVertexArray);
    void destroy();
    ...
    /*!
     @brief
     @param  [in]    locationIndex
     シェーダーに割り当てる頂点ロケーションインデックス
