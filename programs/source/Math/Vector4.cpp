...
Vector4::Vector4(float _n) : x(_n), y(_n), z(_n), w(_n) {}
...
Vector4::Vector4(const Vector4 &src) : x(src.x), y(src.y), z(src.z), w(src.w) {}
...
// ベクトルの定数倍
void Vector4::scale(Vector4 *dst, const Vector4 &src0, const float &src1)
{
    dst->x = src0.x * src1;
    dst->y = src0.y * src1;
    dst->z = src0.z * src1;
