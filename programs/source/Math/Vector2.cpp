...
Vector2::Vector2(const Vector3 &src) : x(src.x), y(src.y) {}

Vector2::Vector2(const Vector4 &src) : x(src.x), y(src.y) {}
...
// ベクトル同士の内積
void Vector2::dot(float *dst, const Vector2 &src0, const Vector2 &src1)
{
    *dst = src0.x * src1.x + src0.y * src1.y;
}

// ベクトル同士の外積
void Vector2::cross(float *dst, const Vector2 &src0, const Vector2 &src1)
{
    // これは2次元の場合
...
// ベクトルの定数倍
void Vector2::scale(Vector2 *dst, const Vector2 &src0, const float &src1)
{
    dst->x = src0.x * src1;
    dst->y = src0.y * src1;
}
